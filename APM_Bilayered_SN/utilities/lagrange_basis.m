...
%
% Modifications:
%       - introduced shape functions for L31, T61, and B20
%           L31, T61 are same as L3 and T6, except that the order of the
%           shape functions are changed.
%--------------------------------------------------------------------------
...
switch type
    ...
    case 'B20'
        %%%%%%%%%%%%%%%%%%% B20 TWENTY NODED BRICK ELEMENT %%%%%%%%%%%%%%%%%%%%
        %
        %                  8
        %               /    \
        %            16         15
        ...
        if size(coord,2) < 3
            ...
            I1=1/2-coord/2;
            I2=1/2+coord/2;
            N=[   -I1(1)*I1(2)*I1(3)*(2+xi+eta+zeta);
                -I2(1)*I1(2)*I1(3)*(2-xi+eta+zeta);
                -I2(1)*I2(2)*I1(3)*(2-xi-eta+zeta);
                -I1(1)*I2(2)*I1(3)*(2+xi-eta+zeta);
                -I1(1)*I1(2)*I2(3)*(2+xi+eta-zeta);
                -I2(1)*I1(2)*I2(3)*(2-xi+eta-zeta);
                -I2(1)*I2(2)*I2(3)*(2-xi-eta-zeta);
                -I1(1)*I2(2)*I2(3)*(2+xi-eta-zeta);
                4*I1(1)*I2(1)*I1(2)*I1(3);
                4*I2(1)*I1(2)*I2(2)*I1(3);
                4*I1(1)*I2(1)*I2(2)*I1(3);
                4*I1(1)*I1(2)*I2(2)*I1(3);
                4*I1(1)*I2(1)*I1(2)*I2(3);
                4*I2(1)*I1(2)*I2(2)*I2(3);
                4*I1(1)*I2(1)*I2(2)*I2(3);
                4*I1(1)*I1(2)*I2(2)*I2(3);
                4*I1(1)*I1(2)*I1(3)*I2(3);
                4*I2(1)*I1(2)*I1(3)*I2(3);
                4*I2(1)*I2(2)*I1(3)*I2(3);
                4*I1(1)*I2(2)*I1(3)*I2(3);  ];
            
            
            dNdxi=[  (eta -1)*(zeta-1)*(eta+2*xi+zeta+1)     (xi-1)*(zeta-1)*(2*eta+xi+zeta+1)        (xi-1)*(eta-1)*(eta+xi+2*zeta+1);
                (eta-1)*(zeta-1)*(-eta+2*xi-zeta-1)     (xi+1)*(zeta-1)*(-2*eta+xi-zeta-1)       (eta-1)*(xi+1)*(-eta+xi-2*zeta -1);
                (eta+1)*(zeta-1)*(-eta-2*xi+zeta+1)     (xi+1)*(zeta-1)*(-2*eta-xi+zeta+1)       (eta+1)*(xi+1)*(-eta-xi+2*zeta+1);
                (eta+1)*(zeta-1)*(eta-2*xi-zeta-1)      (xi-1)*(zeta-1)*(2*eta-xi-zeta-1)        (eta+1)*(xi-1)*(eta-xi-2*zeta-1);
                (eta-1)*(zeta+1)*(-eta-2*xi+zeta-1)     (xi-1)*(zeta+1)*(-2*eta-xi+zeta-1)       (eta-1)*(xi-1)*(-eta-xi+2*zeta-1);
                (eta-1)*(zeta+1)*(eta-2*xi-zeta+1)      (xi+1)*(zeta+1)*(2*eta-xi-zeta+1)        (eta-1)*(xi+1)*(eta-xi-2*zeta+1);
                (eta+1)*(zeta+1)*(eta+2*xi+zeta-1)     (xi+1)*(zeta+1)*(2*eta+xi+zeta-1)       (eta+1)*(xi+1)*(eta+xi+2*zeta-1);
                (eta+1)*(zeta+1)*(-eta+2*xi-zeta+1)     (xi-1)*(zeta+1)*(-2*eta+xi-zeta+1)       (eta+1)*(xi-1)*(-eta+xi-2*zeta+1);
                -4*xi*(eta-1)*(zeta-1)                  -2*(xi-1)*(xi+1)*(zeta-1)                -2*(eta-1)*(xi-1)*(xi+1);
                2*(eta-1)*(eta+1)*(zeta-1)               4*eta*(xi+1)*(zeta-1)                    2*(eta-1)*(eta+1)*(xi+1);
                4*xi*(eta+1)*(zeta-1)                    2*(xi-1)*(xi+1)*(zeta-1)                 2*(eta+1)*(xi-1)*(xi+1);
                -2*(eta-1)*(eta+1)*(zeta-1)             -4*eta*(xi-1)*(zeta-1)                   -2*(eta-1)*(eta+1)*(xi-1);
                4*xi*(eta-1)*(zeta+1)                    2*(xi-1)*(xi+1)*(zeta+1)                 2*(eta-1)*(xi-1)*(xi+1);
                -2*(eta-1)*(eta+1)*(zeta+1)             -4*eta*(xi+1)*(zeta+1)                   -2*(eta-1)*(eta+1)*(xi+1);
                -4*xi*(eta+1)*(zeta+1)                  -2*(xi-1)*(xi+1)*(zeta+1)                -2*(eta+1)*(xi-1)*(xi+1);
                2*(eta-1)*(eta+1)*(zeta+1)              4*eta*(xi-1)*(zeta+1)                    2*(eta-1)*(eta+1)*(xi-1);
                -2*(eta-1)*(zeta-1)*(zeta+1)            -2*(xi-1)*(zeta-1)*(zeta+1)              -4*zeta*(eta-1)*(xi-1);
                2*(eta-1)*(zeta-1)*(zeta+1)             2*(xi+1)*(zeta-1)*(zeta+1)               4*zeta*(eta-1)*(xi+1);
                -2*(eta+1)*(zeta-1)*(zeta+1)            -2*(xi+1)*(zeta-1)*(zeta+1)              -4*zeta*(eta+1)*(xi+1);
                2*(eta+1)*(zeta-1)*(zeta+1)             2*(xi-1)*(zeta-1)*(zeta+1)               4*zeta*(eta+1)*(xi-1);  ]/8;
