...
% infoFaces.extFaces(i,:)=[element1 nface1] for exterior faces (NO dirichlet)
...
for iFace = 1:nOfExteriorFaces
    ...
    nodesFace = T(face(1),EFaces(face(2),:)); %els dels vertexs / [node1 node2]
    ...
    elseif(test==6) %bimaterials, bottom edge fix and DBc at top edge
            if(((abs(XnodesFace(1,2)-1)<1.e-6)&&(abs(XnodesFace(2,2)-1)<1.e-6)) ...
                || ((abs(XnodesFace(1,2)+1)<1.e-6)&&(abs(XnodesFace(2,2)+1)<1.e-6))) %face on dirichlet boundary
            dirichletFaces = [dirichletFaces;face];
        else
            extFaces = [extFaces;face];
            end
    elseif(test==7) % bimaterials with body force
            if(((abs(XnodesFace(1,1)-50)<1.e-6)&&(abs(XnodesFace(2,1)-50)<1.e-6)) ...
                || ((abs(XnodesFace(1,1)+50)<1.e-6)&&(abs(XnodesFace(2,1)+50)<1.e-6))) %face on dirichlet boundary
            dirichletFaces = [dirichletFaces;face];
        else
            extFaces = [extFaces;face];
            end       
    elseif(test==8) %bimaterials, Left edge fix and DBc at right edge
            if(((abs(XnodesFace(1,1)-20)<1.e-6)&&(abs(XnodesFace(2,1)-20)<1.e-6)) ...
                || ((abs(XnodesFace(1,1)+20)<1.e-6)&&(abs(XnodesFace(2,1)+20)<1.e-6))) %face on dirichlet boundary
            dirichletFaces = [dirichletFaces;face];
        else
            extFaces = [extFaces;face];
            end
    elseif(test==9) % 3-point bending, Nguyan et al. 2019
        
%             if((abs(XnodesFace(1,2)+3)==0)&&(abs(XnodesFace(2,2)+3)<1.e-6))
            
            if((abs(XnodesFace(1,2)+3)<1.e-6)&&(abs(XnodesFace(2,2)+3)<1.e-6)&&(abs(XnodesFace(1,1))==10))... %face on dirichlet boundary
                || ((abs(XnodesFace(1,2)+3)<1.e-6)&&(abs(XnodesFace(2,2)+3)<1.e-6)&&(abs(XnodesFace(2,1))==10))
...
nOfDirichletFaces = size(dirichletFaces,1);
