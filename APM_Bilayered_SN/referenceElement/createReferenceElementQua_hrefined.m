...
nOfNodesGeo = size(NodesCoordGeo,1);
...
nodes1d = xs'; 
NodesCoord1dGeo = referenceElementGeo.NodesCoord1d; 
nOfNodes1dGeo = size(NodesCoord1dGeo,1);
...
nIP1d = length(referenceElementGeo.IPweights1d); 
nofnodes1d = length(NodesCoord1d);
...
NxiGeo = referenceElementGeo.Nxi; 
NetaGeo = referenceElementGeo.Neta;
...
a = (1-IPcoordinates1dGeo)/2; 
b = (1+IPcoordinates1dGeo)/2;

for i=1:nOfFaces
    A = NodesCoordGeo(faceNodesGeo(i,1),:); 
    B = NodesCoordGeo(faceNodesGeo(i,end),:);
...
a=(1-IPcoordinates1d)/2; 
b=(1+IPcoordinates1d)/2;

for i=1:nOfFaces
    A=NodesCoord(faceNodes(i,1),:); 
    B=NodesCoord(faceNodes(i,end),:);
...
cellNxiGeofacesIP = cell(nOfFaces,1); 
cellNetaGeofacesIP = cell(nOfFaces,1);
