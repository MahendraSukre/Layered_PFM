function [K,f] = NitscheTwoMeshesDamage_PF(X,T,Xref,Tref,NitscheFaces,refinedElements,referenceElementStd,referenceElementRef,Gc_elem,Gc_elem1,l_elem,betaD,H,Wg,MatTyp,dist,m,M)
...
%[mKe,mKe] = size(Ke)
mKe = size(referenceElementStd.NodesCoord,1);
...
mKe = size(referenceElementRef.NodesCoord,1);
...
for iFace=1:nOfNitscheFaces
    ...
    switch MatTyp
        case 1
            muR = Gc_elem(refElem)*l_elem(refElem);
            muS = Gc_elem(stdElem)*l_elem(stdElem);
            beta = betaD;
        case 2
            if Xe(1,2) < 0
                muR = Gc_elem(refElem)*l_elem(refElem);
    ...
    Ddline_nx = spdiags(dline.*nx,0,ngf,ngf);
    Ddline_ny = spdiags(dline.*ny,0,ngf,ngf);
...
% fe = 2*N'*(dvolu_d*Hg./Gc);
% Ke = l*(Nx'*(dvolu_d*Nx)+ Ny'*(dvolu_d*Ny)) + N'*(spdiags(dvolu.*(1/l + 2*Hg./Gc), 0, ngauss, ngauss)*N);
